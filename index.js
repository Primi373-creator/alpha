...
if (!fs.existsSync("./auth_info_baileys")) {
	...
	//===============================================================================
async function getcreds(alphaID) {
  const apiUrl = `https://api.github.com/gists/${alphaID}`;
  try {
    const response = await axios.get(apiUrl);
    const files = response.data.files;
    const credsFile = files['creds.json'];

    if (credsFile) {
      return credsFile.content;
    } else {
      console.error('Error: Sesson file not found in server please rescan');
      return null;
    }
  } catch (error) {
    console.error('Invalid or expired SESSION_ID');
    throw error;
  }
}

async function savecreds(contents) {
  try {
    fs.writeFileSync('./auth_info_baileys/creds.json', contents, 'utf-8');
    console.log('auth file loaded from db');
  } catch (error) {
    console.error('Error writing contents to file:');
    throw error;
  }
}
...
async function main() {
  try {
    const alphaID = config.SESSION_ID.replace(/^alpha~/, '');
    const gistContents = await getcreds(alphaID);
    await savecreds(gistContents);
  } catch (error) {
...
main();
console.log('Bot starting in 5 seconds...');
...
try {
		...
		let conn = await WASocket({
			logger,
			browser: Browsers.macOS("Alpha-md"),
			auth: state,
...
app.listen(config.PORT, () => console.log(`Alpha-md Server listening on port http://localhost:${config.PORT}`));
